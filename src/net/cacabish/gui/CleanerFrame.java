...
import net.cacabish.module.CleanerModuleController;
import net.cacabish.module.CleanerModuleController.CleanerModuleInfo;
...
public class CleanerFrame extends JFrame {
	...
	private final CleanerModuleController cleanerController = new CleanerModuleController();
	...
	private final FileFilter fileFilter = new FileFilter() {
		 ...
		 * 
		 * @param f the file
		 * @return the file extension of the file. If unsuccessful, this returns an
		 *         empty string.
		 */
	...
	public CleanerFrame() {
		...
		menuItemHowTo.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(contentPane, "First, export your MuseScore sheet as a .musicxml file."
						+ System.lineSeparator() + "This can be loading your sheet in MuseScore. Go to File => Export."
						+ System.lineSeparator()
						+ "Then change the \"Save as type\" to \"Uncompressed MusicXML File (*.musicxml)\" and Save."
						+ System.lineSeparator() + System.lineSeparator() + "Next, load up this program."
						+ System.lineSeparator()
						...
						+ System.lineSeparator() + "You may override the old file or create a new one."
						+ System.lineSeparator() + System.lineSeparator()
						+ "That's it! You can now import into Finale with many annoying bugs fixed! :D", "How To Use",
		...
		menuItemAbout.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(contentPane, new AboutPanel(), "About", JOptionPane.PLAIN_MESSAGE);
			}
		});
			...
			public synchronized void drop(DropTargetDropEvent event) {
				...
				try {
					...
					JOptionPane.showMessageDialog(contentPane,
							"Drag and Drop was unsuccessful. Reason: " + e.getMessage() + System.lineSeparator()
									+ "Try using the button instead.",
							"Drag and Drop Failed", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		JLabel lblDragAndDrop = new JLabel(
				"<html>Drag and drop a .musicxml file to convert here<br> or use the button below.</html>");
...
//		contentPane.add(operationsPanel, BorderLayout.WEST);
		...
		// Add all the modules to the frame
		for (final CleanerModuleInfo moduleInfo : cleanerController.getAllModuleInfo()) {
			JCheckBox moduleCheckbox = new JCheckBox(moduleInfo.moduleFormattedName);
			moduleCheckbox.setToolTipText(moduleInfo.moduleTooltip);
			moduleCheckbox.setSelected(moduleInfo.isEnabled); // Set the default
			moduleCheckbox.addActionListener(new ActionListener() {

				@Override
				public void actionPerformed(ActionEvent e) {
					// When the checkbox is ticked or unticked, update the flag corresponding to the
					// operation.
					if (moduleCheckbox.isSelected()) {
						cleanerController.enableModule(moduleInfo.moduleIndex);
					} else {
						cleanerController.disableModule(moduleInfo.moduleIndex);
					}
				}
			});
			operationsPanel.add(moduleCheckbox);
		}

		// House all the operations in a scroll pane, so as to prevent the need from
		// expanding the window size
		JScrollPane scrollPane = new JScrollPane(operationsPanel);
		scrollPane.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null),
				"Operations to Perform", TitledBorder.LEADING, TitledBorder.TOP, null, null));
					...
					if (status == 200) {
						...
						if (tagMatcher.find()) {
							...
							if (versionTag.compareTo(VERSION) > 0) {
								// There is a new version! Make the button appear and construct the window to
								// appear!
								...
								EventQueue.invokeLater(new Runnable() {
									...
									public void run() {
										...
										newVersionButton.addActionListener(new ActionListener() {
											...
											public void actionPerformed(ActionEvent e) {
												JOptionPane.showMessageDialog(contentPane,
														new NewVersionPanel(versionTag), "New Version Available!",
	...
	/**
	 * Shows a JFileChooser with the MusicXML FileFilter and then loads the file
	...
	private void loadFile(File fileToLoad) {
		if (!fileFilter.accept(fileToLoad)) {
			JOptionPane.showMessageDialog(this,
					fileToLoad.getName() + " is not a .musicxml or .xml file." + System.lineSeparator()
							+ "Double check the file extension on the file.",
		...
		try {
			// Do the cleaning
			cleanerController.cleanFile(fileToLoad);
			...
			JOptionPane.showMessageDialog(this, "Unable to parse " + fileToLoad.getName()
					+ ". Please let cacabish know about this!" + " Reason: " + e.toString(), "Failed to Parse File",
	...
	/**
	 * Saves the XML document (cached in the cleaner) to a file after prompting the
	 * user for a save location.
	 * 
	 * @param parentFile the original file that the document came from. Gives quick
	 *                   access to simply override the original file.
	 */
	private void saveFile(File parentFile) {
			...
			public void approveSelection() {
				...
				if (f.exists()) {
					...
					int confirmation = JOptionPane.showConfirmDialog(contentPane,
							"This file exists. Do you want to overwrite this file?", "Overwrite Existing File?",
		...
		if (response == JFileChooser.APPROVE_OPTION) {
			...
			if (chosenFile == null) {
				// Don't know how we got here, but it's bad news if this happens. Abort for
				// safety.
				JOptionPane.showMessageDialog(contentPane, "Save was unsuccessful. Reason: file was null",
			...
			// Save the file!
			try {
				cleanerController.saveLastCleanedDocumentToFile(parentFile);

				// Optional: Notify the user of the success
				JOptionPane.showMessageDialog(contentPane, "Successfully saved to " + chosenFile.getName(),
						"Save Successful", JOptionPane.INFORMATION_MESSAGE);
			} catch (Exception e) {
				// If anything goes wrong, report the failure.
				JOptionPane.showMessageDialog(contentPane, "Save was unsuccessful. Reason: " + e.getMessage(),
						"Save Failed", JOptionPane.ERROR_MESSAGE);
				return;
			}
		} else {
			// They chose cancel, no, or closed the window. In all these cases, and any
			// others unaccounted for, abort.
	...
	private static class AboutPanel extends JEditorPane {
		 ...
		 */
		public AboutPanel() {
			super("text/html", "<html>" + "<body style='font-family: Tahoma; font-size: 14;'>" + "<b>MusicXML Cleaner "
					+ VERSION + " for MuseScore " + MUSESCORE_VERSION + "</b>" + "<br>"
					...
					+ "<br><br>" + "Copyright (c) 2020-23 cacabish" + "<br>"
					+ "MusicXML 3.1 by W3C Music Notation Community Group" + "<br>" + "Published under the MIT License"
					+ "</body>" + "</html>");

			// This adds a listener so that when a link is clicked, it attempts to browse
			// that link.
	...
	private static class NewVersionPanel extends JEditorPane {
		...
		public NewVersionPanel(String newVersion) {
			super("text/html",
					"<html>" + "<body style='font-family: Tahoma; font-size: 14;'>" + "A new version is available!"
							+ "<br><br>" + "<b>Current Version:</b> " + VERSION + "<br>" + "<b>New Version:</b> "
							+ newVersion + "<br><br>"
